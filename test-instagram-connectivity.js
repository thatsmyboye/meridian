...
async function testGraphApiConnectivity() {
  ...
  try {
    ...
    console.error(`   ❌ Failed to reach Meta Graph API: ${error.message}`);
    return false;
  }
}

async function testTokenRefreshMechanism(config) {
  console.log("\n🔄 Testing Token Refresh Mechanism...");

  if (!config.metaAppId || !config.metaAppSecret) {
    console.log("   ⚠️  Skipped: META_APP_ID/SECRET not configured");
    return false;
  }
  ...
  try {
    ...
    if (response.status === 400 || response.status === 401) {
      console.log("   ✅ Token refresh endpoint is reachable (validation working)");
      return true;
    }
    ...
    console.error(`   ❌ Failed to reach token refresh endpoint: ${error.message}`);
    return false;
  }
}

async function testMediaEndpoint(config) {
...
async function testInsightsEndpoint(config) {
  console.log("\n📊 Testing Insights Endpoint...");

  if (!config.testAccessToken) {
    console.log("   ⚠️  Skipped: Test token not configured");
    return false;
  }

  const result = await makeGraphApiRequest(`test_media_id/insights`, {
  ...
  if (result.status === 400 || result.status === 404 || result.status === 403) {
    console.log("   ✅ Insights endpoint is reachable (endpoint structure validated)");
    return true;
  } else if (result.ok) {
    console.log("   ✅ Insights endpoint is accessible");
    return true;
  } else {
    console.error(`   ❌ Insights endpoint error (${result.status}): ${result.error}`);
    return false;
  }
}

function validateTokenRefreshLogic() {
  console.log("\n🔐 Validating Token Refresh Logic...");

  try {
    const exampleExpiry = new Date();
    exampleExpiry.setDate(exampleExpiry.getDate() + 10);
    ...
    if (!shouldRefresh) {
      ...
      console.log(`      Threshold (7-day buffer): ${threshold.toISOString()}`);
      console.log(
...
function validateMediaTypeHandling() {
  console.log("\n📋 Validating Media Type Handling...");

  const mediaTypes = ["IMAGE", "VIDEO", "CAROUSEL_ALBUM", "REEL"];
  ...
  try {
    ...
    console.error(`   ❌ Media type validation failed: ${error.message}`);
    return false;
  }
}

function validateMetricsNormalization() {
  ...
  try {
    ...
    const engagementRate =
      testMetrics.views > 0 ? interactions / testMetrics.views : 0;
    ...
    if (
      ...
      console.log(
        `      Raw: views=${testMetrics.views}, interactions=${interactions}`
      );
      console.log(
    ...
    console.error(
      `   ❌ Metrics normalization validation failed: ${error.message}`
    );
    return false;
  }
...
function validateThumbnailSelection() {
  ...
  try {
    ...
    console.error(
      `   ❌ Thumbnail selection validation failed: ${error.message}`
    );
    return false;
  }
...
async function runAllTests() {
  ...
  console.log("\n" + "─".repeat(66));
  console.log(`Result: ${passedCount}/${testSummary.length} core tests passed\n`);
